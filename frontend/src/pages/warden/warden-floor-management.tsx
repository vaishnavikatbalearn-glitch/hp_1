...
export function WardenFloorManagement() {
  ...
  const floors = [
    { number: 1, totalRooms: 20, occupied: 18, available: 2 },
    { number: 2, totalRooms: 20, occupied: 20, available: 0 },
    { number: 3, totalRooms: 20, occupied: 15, available: 5 },
  ];
  return (
              ...
              <Card key={i} className="bg-card border-border">
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><p className="text-xs text-muted-foreground">Occupied</p><p className="text-lg">{f.occupied}</p></div>
                    <div><p className="text-xs text-muted-foreground">Available</p><p className="text-lg text-green-600">{f.available}</p></div>
                  </div>
                </div>
              </Card>
            ))}
