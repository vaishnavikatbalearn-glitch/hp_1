...
export function WardenInitiativeModeration() {
  const navigate = useNavigate();
  const initiatives = [
    { type: 'Startup Idea', student: 'Rahul Sharma', title: 'AI-based Study Assistant', description: 'Mobile app for personalized learning' },
  ];
  return (
            ...
            <Card key={i} className="bg-card border-border">
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <Badge className="bg-purple-500">{init.type}</Badge>
                </div>
                <p className="text-sm mb-1">{init.title}</p>
                <p className="text-xs text-muted-foreground mb-3">By {init.student}</p>
                <p className="text-sm mb-4">{init.description}</p>
